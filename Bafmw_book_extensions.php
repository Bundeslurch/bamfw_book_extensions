...
/**
 * Plugin Name:     Bafmw_book_extensions
...
define('BAE_PATH', plugin_dir_path(__FILE__));
...
add_action('woocommerce_single_product_summary', 'add_author_box');
...
function add_author_box(){
    ...
    foreach($authors as $author){
			$user_firstname = get_user_meta( $author['ID'], 'first_name', true );
	$user_lastname = get_user_meta( $author['ID'], 'last_name', true );
        $template = '<a href="%s"/>%s %s</a> ';
        echo sprintf($template,get_author_posts_url($author['ID']), $user_firstname ,$user_lastname  );
    }
    echo '</span>';
endif;
}